...
Clojure是JVM上的一个LISP语言变种。它比Common Lisp更强调纯函数式编程，但提供了一些STM工具（代码自动生成）以处理它所引入的状态问题。这个组合使并行处理更加简单，并且通常是自动的。
...
; 从一个map中检索一个值，可以直接把这个map当做函数调用
(stringmap "a") ; => 1
(keymap :a) ; => 1

; 关键字也可以当做函数来调用，从一个map中检索值
(:b keymap) ; => 2
...
; 使用 doto 来处理可变的类，所有的函数始终用最初的那个对象值，最后还是返回最初的那个对象  
(import java.util.Calendar)
(doto (Calendar/getInstance)
  (.set 2000 1 1 0 0 0)
  .getTime) ; => A Date. set to 2000-01-01 00:00:00

; 本文地址：https://www.oschina.net/translate/learn-clojure-in-minutes

; 原文地址：http://learnxinyminutes.com/docs/clojure/
